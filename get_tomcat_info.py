#!/usr/bin/env python3
"""
get_tomcat_info_csv_v3.py

Read-only CloudShell script. Interactive selection of EC2 instance(s), sends
a read-only SSM Run Command to find Tomcat server.xml files and extract only
the redirectPort value. Maps listening process info (if any) for that redirect port.
...
Notes:
- Read-only on instances: uses find/grep/ss/cat/stat/etc only.
- Requires IAM: ec2:DescribeInstances, ssm:SendCommand, ssm:GetCommandInvocation, sts:GetCallerIdentity.
"""
import boto3
import csv
import datetime
...
# Strictly read-only script run on instance via SSM
SSM_SCRIPT = r'''
#!/bin/bash
set -u

echo "SSM_TOMCAT_SCAN_START"
# Restrict find to typical per-user apache-tomcat folders under /home to reduce noise
find /home -type f -path "*/apache-tomcat*/conf/server.xml" 2>/dev/null | sort -u | while read -r f; do
  tomcat_home="$(dirname "$(dirname "$f")")"
  # guess user from /home/<user>/...
  user="$(echo "$f" | awk -F/ '{print $3}' 2>/dev/null || echo '')"
  if [ -r "$f" ]; then
    # Extract redirectPort only (if any). Output a single line per server.xml with redirectPort (may be empty)
    # we still capture the full server.xml path and tomcat_home and user
    # If multiple Connector entries have redirectPort, print them all (one per line)
    grep -oP '<Connector[^>]*>' "$f" 2>/dev/null | while read -r conn; do
      ...
      echo "SERVERXML|$f|$tomcat_home|$user|$redirectPort"
    done
  fi
done

# Also check /opt, /srv, /var for server.xml under conf if some installs are there
find /opt /srv /var -type f -path "*/conf/server.xml" 2>/dev/null | sort -u | while read -r f; do
  tomcat_home="$(dirname "$(dirname "$f")")"
  user="$(stat -c '%U' "$tomcat_home" 2>/dev/null || echo '')"
  grep -oP '<Connector[^>]*>' "$f" 2>/dev/null | while read -r conn; do
    redirectPort="$(echo "$conn" | sed -n 's/.*redirectPort="\([^"]*\)".*/\1/p' || true)"
    [ -z "$redirectPort" ] && redirectPort=""
    echo "SERVERXML|$f|$tomcat_home|$user|$redirectPort"
...
# Produce listening sockets map: LISTEN|port|pid|uid|cmd
...
def list_instances(ec2):
    resp = ec2.describe_instances(Filters=[{'Name':'instance-state-name','Values':['running','stopped','stopping','pending']}])
    insts = []
...
def choose_instances(instances):
    ...
    if not picked:
        p("No valid instances selected. Exiting."); sys.exit(1)
    return list(dict.fromkeys(picked))

def send_ssm(ssm, instance_ids):
    resp = ssm.send_command(InstanceIds=instance_ids, DocumentName='AWS-RunShellScript', Parameters={'commands':[SSM_SCRIPT]}, TimeoutSeconds=300)
...
def poll_result(ssm, iid, cmd_id, timeout=300, interval=3):
    ...
    while True:
        try:
            ...
            if elapsed>timeout: return {'Status':'TimedOut','Stdout':'','Stderr':'Timeout waiting'}
            continue
        status = out.get('Status','')
        if status in ('Pending','InProgress','Delayed'):
            time.sleep(interval); elapsed+=interval
            if elapsed>timeout: return {'Status':'TimedOut','Stdout':out.get('StandardOutputContent',''),'Stderr':out.get('StandardErrorContent','')}
            continue
        return {'Status':status,'Stdout':out.get('StandardOutputContent',''),'Stderr':out.get('StandardErrorContent','')}

def parse_output(stdout):
    # SERVERXML|serverxml_path|tomcat_home|user|redirectPort
    server_rows=[]
    listen_map={}
    for line in stdout.splitlines():
        ...
        if line.startswith("SERVERXML|"):
            parts=line.split("|",5)
            if len(parts)>=5:
                _, serverxml_path, tomcat_home, user, redirectPort = parts[:5]
                server_rows.append({'serverxml_path':serverxml_path,'tomcat_home':tomcat_home,'user':user,'redirect_port':redirectPort})
    ...
    # attach proc info based on redirect_port
    for r in server_rows:
        rp = r.get('redirect_port','')
        mappings = listen_map.get(rp, [])
        if mappings:
            r['proc_pid'] = mappings[0].get('pid','')
            r['proc_uid'] = mappings[0].get('uid','')
            r['proc_cmd'] = mappings[0].get('cmd','')
        else:
            r['proc_pid']=''; r['proc_uid']=''; r['proc_cmd']=''
    return server_rows
...
def main():
    ...
    picked = choose_instances(instances)

    try:
        cmd_id = send_ssm(ssm, picked)
    ...
    for iid in picked:
        ...
        for r in parsed:
            ...
            if rp:
                if public_ip:
                    domainwithporturl = f"{public_ip}:{rp}"
                elif private_ip:
                    domainwithporturl = f"{private_ip}:{rp}"
            # connector_port intentionally left blank per your requirement to only use redirectPort
            all_rows.append({
